...
class _ScreenOnScreenState extends ConsumerState<LockScreen> {
  ...
  double _maxDrag = 400;
  late final StreamSubscription<ScreenStateEvent> _screenSubscription;
  ...
  DataInit() async {
    ...
    await ref.read(TimeStateNotifierProvider.notifier).UpdateTime();
  }
  ...
  void initState() {
    ...
    _screenSubscription = screen.screenStateStream.listen((event) {
      if (event.name == "android.intent.action.SCREEN_OFF") {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) return;
          context.go(LockScreen.routeFullPath);
        });
      }
    });
  }
